...
md`# Gráfico de Bolhas

Os gráficos de bolhas são círculos compactados não hierárquicos . A área de cada círculo é proporcional ao seu valor (aqui, tamanho do arquivo). A aparência orgânica desses diagramas pode ser intrigante, mas considere também um mapa de árvore ou um gráfico de barras simples. `
)}
...
BubbleChart(files, {
  ...
  title: d => `${d.id}\n${d.value.toLocaleString("pt")}`,
  //link: d => `https://github.com/prefuse/Flare/blob/master/dados/src/${d.id.replace(/\./g, "/")}.as`,
  width: 1152
})
)}

function _flare(FileAttachment){return(
FileAttachment("dados.csv").csv({typed: true})
)}

function _files(dados){return(
dados.filter(d => d.value !== null)
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["dados.csv", {url: new URL("dados.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  ...
  main.variable(observer("dados")).define("dados", ["FileAttachment"], _flare);
  main.variable(observer("files")).define("files", ["dados"], _files);
